...
Now we need to estimate the quanity of points in an intersection of an integral point set and a line segment.
...
\begin{proof}
	Any $n^2+1$ points, including the ends of $\Delta$, split the segment $\Delta$ into $n^2$
...
\begin{problem}
	...
	Is there a number $\delta < 1$ such that any planar integral point set $M$ with $\operatorname{diam} M = d$
	belongs to a square of side length $d^\delta$?
\end{problem}
If the answer for Problem~\ref{prb:square_side_length_and_diameter_power} is affirmative,
then we can build an estimate better than linear.

\begin{problem}
	\label{prb:square_side_length_and_diameter_linear}
	What are the maximal numbers $\delta \leq 1$ and $h \leq 0$
	such that any planar integral point set $M$ with $\operatorname{diam} M = d$
	belongs to a square of side length $\delta d - h$?
\end{problem}

If the answer for Problem~\ref{prb:square_side_length_and_diameter_linear} is not $\delta = 1$ and $h = 0$,
then the linear estimate can be slightly improved.

However, Problems~\ref{prb:square_side_length_and_diameter_power} and~\ref{prb:square_side_length_and_diameter_linear}
do not seem to show the way for improving the estimate
due to the following well-known theorem (TODO: refs!).
\begin{theorem}
...
Another possibility to increase the constant in out lower bound is introduced by the following problem.

\begin{problem}
	\label{prb:minimal_shape}
	What is the minimal shape $\mathcal{S}$ such that any planar integral point set $M$ with $\operatorname{diam} M = d$
	belongs to $\mathcal{S}$?
\end{problem}
This problem in another generalization of Lemma~\ref{lem:square_container}.
...
The solution of Problem~\ref{prb:minimal_shape} (if ever found) will lead to sophisticated packing problem,
...
When we talk about integral point sets containing distance 1, another problem arises.
...
For greater $m$, we will be brave enough to put forward
\begin{conjecture}
	...
	not more than $1+\frac{m^2 - m}{2}$ times.
\end{conjecture}
Such as estimate is based on the quantity of edges in $(m-2)$-dimensional simplex.

We want to end the list of conjectures with the one which looks the easiest to deal with.
...
\begin{enumerate}
	...
	\item
		know out - no usage found!

	\item
		contradict with or to ?
	...
	\item
		estimate vs. bound vs. estimation
