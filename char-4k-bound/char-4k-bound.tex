...
\paragraph{Abstract.}
A point set $M$ in a Euclidean plane is called an integral point set in semi-general position if all the distances between the
elements of $M$ are integers, and $M$ does not contain collinear triples.
In this paper we improve the lower bound for the diameter of such sets in the particular case
...
\section{Introduction}
	...
	however, Erd{\H o}s did not only estimate the diameter,
	...
	All these lower bounds are based on Erd{\H o}s's framework: cofocal hyperbolas.


	Meanwhile in 1988, Kemnitz~\cite{kemnitz1988punktmengen} introduced a characteristic for an IPS.
	The charateristic of an IPS is the one and only squarefree integer $q$ which is comparable
	with the area of the triangle formed by any triple of points from the IPS.
	...
	(Taking the characteristic into consideration allows one to boost such search significantly; this is
	basically because all triples of an IPS form triangles with equal characteristic.)

	For generalization to higher dimensions, we refer the reader to~\cite{nozaki2013lower}.

	In the present paper, we combine the power of Erd{\H o}s's and Kemnitz's approaches together. Due to the squarefree nature of the characteristic, it can be of the form $4k+1$, $4k+2$ or $4k+3$, where $k$ is integer.
	...
	\begin{equation}
		\operatorname{diam} M \ge \left( \frac{25}{36} \cdot \# M \right)^{5/4}
		.
	\end{equation}

	In Section 2, we give all the required notions and known results. In Section 3, we discuss some examples of integral point sets in order to demonstrate that none of the classes $4k+1$, $4k+2$, $4k+3$ is too exotic nor pathological. Section 4 describes a connection between Erd{\H o}s curves and the characteristic. In Section 5, we prove some auxiliary results, and we proceed to the main one in Section 6.
...
For the sake of brevity, the following notation is used for sets
of positive integers, non-negative integers and all the integers respectively:
\begin{equation}
...
However, the next restriction step can be done as follows.
...
\begin{definition}
	...
	\end{equation}
	the \emph{$n$-th Erd{\H o}s curve}.
\end{definition}

Obviously, the 0-th Erd{\H o}s curve is the perpendicular bisector of $M_1 M_2$,
and all the other Erd{\H o}s curves are branches of cofocal hyperbolas.
In Figure~\ref{fig:Erdos_curves_numbered}, the Erd{\H o}s curves are shown for an edge of length 3.

\begin{figure}
\center{\includegraphics[width=0.95\linewidth]{Erdos_curves_numbered.png}}
\caption{Erd{\H o}s curves}
\label{fig:Erdos_curves_numbered}
\end{figure}

Thus, an edge $M_1 M_2$ generates $2|M_1 M_2| - 1$ Erd{\H o}s curves.
For the sake of brevity, Erd{\H o}s curves with odd numbers are named odd Erd{\H o}s curves,
and the ones with even numbers are named even Erd{\H o}s curves.
...
the very first example that comes to our mind is the Egyptian triangle,
that is the triangle with sides $(3;4;5)$. The
...
For the sake of completeness, in Figure~\ref{fig:rails_3_8.png}
...
%4
\section{Erd{\H o}s Curves and Characteristic}
...
\begin{theorem}[The Weeding Theorem I]
	...
	ant let $M_3$ belong to the $n$-th Erd{\H o}s curve of the edge $M_1 M_2$.
...
\begin{theorem}[The Weeding Theorem II]
	...
	Let an IPS $M=\{M_1, M_2, M_3\}\in{\mathfrak{M}_{3}}$ have an even edge length $|M_1 M_2|$, and let point $M_3$ lie on an odd Erd{\H o}s curve of edge $M_1 M_2$.
	Then $\operatorname{char} M=8k+7$, $k\in \mathbb{N}_{0}$.
\end{theorem}

\begin{proof}
	Indeed, let $|M_1 M_2|=2q$, $q \in \mathbb{N}$.
	For the $n$-th Erd{\H o}s curve we have $|M_1 M_3|-|M_2 M_3|=n$.
...
\begin{proof}
	...
	(s-k)(s+k)=1.
	\end{equation}
	Since $s,k\in\mathbb{N}$, the number 1 can be represented as the product of two integers in two ways: $1\cdot1$ and $-1\cdot(-1)$. In the first case, $(s-k)=1$ and $(s+k)=1$, which gives $s=1,k=0$. This contradicts the fact that $k\in\mathbb{N}$.
	In the second case, $(s-k)=-1$ and $(s+k)=-1$, which gives $s=-1$, $k=0$. This also contradicts $s,k\in\mathbb{N}$.
...
The following result is a classical Erd{\H o}s-style intersection-enumeration lemma,
...
\begin{equation}
	\begin{array}{rcl}
		...
		\overline{d}(2,10)&=& \dots =\overline{d}(2,12)= 105,{} \\
		\overline{d}(2,13)&=&  \overline{d}(2,14)= 532,{}\\
		\overline{d}(2,15)&=& \dots = \overline{d}(2,18) = 735,{} \\
		\overline{d}(2,19)&=& \dots = \overline{d}(2,24) = 1995,{} \\
		\overline{d}(2,25)&=&  \dots = \overline{d}(2,27)= 9555,{} \\
		\overline{d}(2,28)&=& 10672,{} \\
		\overline{d}(2,29)&=&  \dots = \overline{d}(2,36) = 13975,{} \\
...
\section{Conclusion}
	The bound proved above (as well as~\eqref{eq:chebsb_lower_bound}) may appear to be far from the precise values of $\overline{d}(2,n)$.
	However, it is easy to see that values of $d(2;n)$ tend to repeat often;
...
Also the authors owe special thanks to the anonymous referees
...
N. H. Anning and P. Erd{\H o}s, Integral distances, \emph{Bull. Amer. Math. Soc.} 51.\textbf{8} (1945), 598--600, \textsc{doi}: \href
...
\it{Proceedings of Voronezh State University. Series: Physics. Mathematics}. \textbf{1} (2025), 62--77.
...
P. Erd{\H o}s, Integral distances, \emph{Bull. Amer. Math. Soc.} 51.\textbf{12} (1945), 996, \textsc{doi}: \href
...
A. Kemnitz, Point sets with integer distances (Punktmengen mit ganzzahligen Abständen), 1988.
%
...
J. Solymosi and F. De Zeeuw, On a question of Erd{\H o}s and Ulam, \emph{Discrete Comput. Geom.} 43.\textbf{2} (2010), 393--401,
arXiv: \href {http://arxiv.org/abs/0806.3095} {\nolinkurl {0806.3095}}.
%
\bibitem{our-ped-2018}
N. N. Avdeev, On integral point sets in special position,
...
(Об отыскании целоудалённых множеств специального вида),
...
M. Smurov and A. Spivak, Covering by strips (Покрытия полосками), \emph{Kvant} \textbf{5} (1998), 6--12.
\end{thebibliography}
