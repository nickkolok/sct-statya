...
\section{Introduction}
	...
	In 1945, Erdős gave an elegant proof~\cite{anning1945integral,erdos1945integral} that every IPS is finite.
	...
	In the present paper, we put the power of Erdős's and Kemnitz's approaches together. Due to squarefree nature of characteristic, it can be of the form $4k+1$, $4k+2$ or $4k+3$, where $k$ is integer.
	...
	In Section 2, we give all the required notions and known results. In Section 3, we discuss some examples of integral point sets in order to demonstrate that none of the classes $4k+1$, $4k+2$, $4k+3$ is too exotic nor pathological. Section 4 is devoted to the connection between Erdős curves and the characteristic. In Section 5, we prove some auxiliary results, and we proceed to the main one in Section 6.
...
\begin{definition}
	...
	we will call the set of points
	\begin{equation}
...
\begin{theorem}[The Weeding Theorem I]
	\label{thm:weeding_I}
	Let $M = \{M_1, M_2, M_3\} \in \mathfrak{M}_3$ be an IPS with a characteristic different from $4k+3$, $k \in \mathbb{N}_0$. Then, the length of the edge $M_1 M_2$ and its $n$-th Erdős curve, which contains the point $M_3$, have the same parity.
\end{theorem}
...
\begin{proof}
	Indeed, for $a\geq 3$ the inequality
	$$
...
The following lemma is a classical Erdős-style intersection-enumeration one,
...
Now we are finally ready to proove our main result.
