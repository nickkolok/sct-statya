...
We improve the linear lower bound for diameter of such sets in the particular case
...
\section{Introduction}
	...
	it can be inferred from his proof that $c=\frac{1}{24}$ [TODO].
	...
	of any triangle formed by any triple of points from the IPS is comparable with $\sqrt{q}$.
	(Indeed, Kemnitz proved this fact for any point set with rational distances ---
	in contrast with IPS, those can be infinite even they contain non-collinear triples,
	see~\cite{huff1948diophantine} for an example of construction
	...
	basically because all triples of an IPS form triangles with equal characteristics.)

	For generalization in higher dimensions, we refer the reader to~\cite{nozaki2013lower}.

	In the present paper, we put the power of Erdös and Kemnitz approaches together. Due to squarefree nature of characteristic, it can be of the form $4k+1$, $4k+2$ or $4k+3$, where $k$ is integer.
...
\begin{definition}
	...
	for any pair of points $M_{1}, M_{2} \in \mathcal{P}$
...
\begin{definition}
	If points $M_1, M_2 \in M \in \mathcal{M}$,
	then the line segment $M_1 M_2$ is said to be an \emph{edge} of $M$.
\end{definition}



The following result is due to Kemnitz:
...
For the sake of completeness, we provide the proof for the Grid Theorem below.
...
\begin{figure}
\center{\includegraphics[width=0.6\linewidth]{Erdos_curves_numbered.png}}
...
As far as our attention will be mostly restricted to planar integral point sets
with characteristic of the form $4k+1$ and $4k+2$ in semi-general position.
If an IPS $M$ satisfies these conditions,
we will write that
...
\begin{figure}
\center{\includegraphics[width=0.6\linewidth]{char385.png}}
...
Also the largest known rails IPS presented in~\cite{}[TODO:Omsk!]
that has 104 points on one straight line and the rest 2 on another
(that is the cardinality of 106) has the characteristic of $154 = 4 \cdot 38 + 2$.
(We do not list the coordinates of that set here due to its diameter which is 2745754098774581800288844387372160.)

\begin{figure}
\center{\includegraphics[width=0.6\linewidth]{heptagon2.png}}
...
Moreover, all the IPS of minimal possible diameter provided in~\cite[\S 5, Fig. 1]{harborth1993upper}
(for cardinalities from 3 to 9) have characteristic of form $4k+3$.

For the sake of completeness, on Fig.~\ref{fig:11_2432_255255_a0a1b7f805cfc93080b4608505a0f45d_rails_3.png}
we give an example of rails set
with 3 points on one line and 8 points on the other (first presented in~\cite{})
...
\begin{figure}
\center{\includegraphics[width=0.6\linewidth]{11_2432_255255_a0a1b7f805cfc93080b4608505a0f45d_rails_3.png}}
\caption{The heptagon in general position}
...
\begin{proof}
	...
	\label{eq: 32}
	\end{equation}
	Substitute the obtained expression (\ref{eq:32}) into the second equation of the system (\ref{eq:30}).
	\begin{equation}
	...
	\label{eq: 35}
	\end{eqnarray}
	For equation (\ref{eq:35}) to have integer solutions, it is necessary for $p$ and $b$ to be odd.
...
\begin{proof}
	...
	If $m$ is an odd number, then by Remark \ref{rm: 2}, the difference $n = |M_1 M_3| - |M_2 M_3| = m - s$, where $s$ is an even number, is also an odd number.
...
\begin{proof}
	...
	For $s=1$, the conditions of TODO are satisfied: $s$ is an odd number and $s \leq a$.
...
\section{Conclusion}
	The bound proved above (as well as the one (TODO)) may appear to be far from precise values of $\overline{d}(2,n)$.
